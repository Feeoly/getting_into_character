...
function toRecordingError(err: unknown): RecordingError {
  ...
  if (name === "NotAllowedError" || name === "SecurityError") {
    return {
      code: "not_allowed",
      message: "未获得麦克风/摄像头权限。请在浏览器权限设置中允许后重试。",
      cause: err,
    };
  }
...
export async function startRecording(
  ...
  const kind: RecordingKind = input.cameraEnabled ? "video" : "audio";
  const constraints: MediaStreamConstraints = input.cameraEnabled
